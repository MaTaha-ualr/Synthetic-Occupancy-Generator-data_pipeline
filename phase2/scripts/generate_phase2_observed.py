...
def _build_emission_yaml_payload(emission_cfg: Any) -> dict[str, Any]:
    ...
    if is_legacy:
        ...
        payload["record_count_A"] = payload.pop("record_count_a")
        payload["record_count_B"] = payload.pop("record_count_b")
        payload["noise"] = {
    ...
    payload.pop("record_count_a", None)
    payload.pop("record_count_b", None)
...
def main() -> int:
    ...
    progress = ProgressReporter(
        total=5,
        label="Phase 2 observed",
        enabled=not args.no_progress,
    )

    with progress.step("Load run and scenario configuration"):
        ...
        truth_events_path = artifact_paths["truth_events"]
        for required in (truth_people_path, truth_residence_path, truth_events_path):
        ...
        master_dataset_path = artifact_paths["master_dataset"]
        crosswalk_path = artifact_paths["truth_crosswalk"] if len(emission_cfg.datasets) == 2 else None
        if not args.overwrite:
            observed_targets = [
                entity_record_map_path,
                master_dataset_path,
                *observed_dataset_paths.values(),
                *pairwise_crosswalk_paths.values(),
            ]
    ...
    with progress.step("Emit observed datasets"):
    ...
    with progress.step("Write observed outputs and metadata"):
        ...
        emitted["master_dataset"].to_csv(master_dataset_path, index=False)
    ...
    with progress.step("Compute quality report"):
        ...
        # Preserve the constraints check carried over from the truth step so the observed
        # step cannot silently upgrade a constraint-violating run back to "ok"
        # (mirrors sog_phase2.pipeline._compute_quality_status).
        constraints_valid = bool((quality.get("constraints_validation") or {}).get("valid", True))
        status = (
            "ok"
    ...
    result = {
        ...
        "master_dataset_rows": int(len(emitted["master_dataset"])),
        "master_source_rows": int(emitted["metrics"].get("master_source_rows", 0)),
        "master_timeline_rows": int(emitted["metrics"].get("master_timeline_rows", 0)),
