...
def _progress_step(progress: Any | None, message: str):
    if progress is None:
        return nullcontext()
    return progress.step(message)


def build_phase2_params(output_dir: Path, progress: Any | None = None) -> dict[str, object]:
    with _progress_step(progress, "Initialize output directory"):
    ...
    with _progress_step(progress, "Fetch ACS mobility data"):
    ...
    with _progress_step(progress, "Write mobility parameter tables"):
    ...
    with _progress_step(progress, "Fetch ACS household data"):
    ...
    with _progress_step(progress, "Write household and demographic tables"):
        household_rows = [
            (
    ...
    with _progress_step(progress, "Write priors snapshot and manifest"):
        priors = {
...
def main() -> int:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=project_root / "phase2" / "Data" / "phase2_params",
    ...
    progress = ProgressReporter(
        total=6,
        label="Phase 2 params",
        enabled=not args.no_progress,
    )
    manifest = build_phase2_params(args.output_dir.resolve(), progress=progress)
