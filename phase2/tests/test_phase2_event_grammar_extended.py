...
# ---------------------------------------------------------------------------
# LIFECYCLE: DEATH / NAME_CHANGE / ADOPTION
# ---------------------------------------------------------------------------

def test_valid_death_event() -> None:
    row = _blank_row(
        EventKey="E1", EventType="DEATH", EventDate="2026-06-01",
        SubjectPersonKey="1", SubjectHouseholdKey="H1", FromAddressKey="A1",
    )
    result = validate_truth_events_dataframe(pd.DataFrame([row]))
    assert result["valid"] is True


def test_death_missing_subject() -> None:
    row = _blank_row(EventKey="E1", EventType="DEATH", EventDate="2026-06-01")
    result = validate_truth_events_dataframe(pd.DataFrame([row]))
    assert result["valid"] is False
    assert any("SubjectPersonKey" in err["field"] for err in result["errors"])


def test_valid_name_change_event() -> None:
    row = _blank_row(
        EventKey="E1", EventType="NAME_CHANGE", EventDate="2026-07-01",
        SubjectPersonKey="1",
        PreviousFirstName="Ava", PreviousMiddleName="", PreviousLastName="Stone",
        PreviousFullName="Ava Stone",
        NewFirstName="Ava", NewMiddleName="", NewLastName="Parker",
        NewFullName="Ava Parker",
...
def test_name_change_rejects_unchanged_full_name() -> None:
    row = _blank_row(
        EventKey="E1", EventType="NAME_CHANGE", EventDate="2026-07-01",
        SubjectPersonKey="1",
        PreviousFullName="Ava Stone",
        NewFullName="Ava Stone",
        NameChangeReason="legal_name_change",
    )
    result = validate_truth_events_dataframe(pd.DataFrame([row]))
    assert result["valid"] is False
...
def test_name_change_requires_reason() -> None:
    row = _blank_row(
        EventKey="E1", EventType="NAME_CHANGE", EventDate="2026-07-01",
        SubjectPersonKey="1",
        PreviousFullName="Ava Stone",
    ...
    result = validate_truth_events_dataframe(pd.DataFrame([row]))
    assert result["valid"] is False
    assert any("NameChangeReason" in err["field"] for err in result["errors"])


def test_valid_adoption_event() -> None:
    row = _blank_row(
        EventKey="E1", EventType="ADOPTION", EventDate="2026-08-01",
        ChildPersonKey="C1",
        AdoptiveParent1PersonKey="1",
        AdoptiveParent2PersonKey="2",
        NewHouseholdKey="H2",
        FromAddressKey="A1",
        ToAddressKey="A2",
    )
...
def test_adoption_rejects_child_as_adoptive_parent() -> None:
    row = _blank_row(
        EventKey="E1", EventType="ADOPTION", EventDate="2026-08-01",
        ChildPersonKey="C1",
        AdoptiveParent1PersonKey="C1",
        NewHouseholdKey="H2",
    )
    result = validate_truth_events_dataframe(pd.DataFrame([row]))
    assert result["valid"] is False
...
def test_adoption_requires_new_household() -> None:
    row = _blank_row(
        EventKey="E1", EventType="ADOPTION", EventDate="2026-08-01",
        ChildPersonKey="C1",
        AdoptiveParent1PersonKey="1",
...
def test_multiple_events_in_one_dataframe() -> None:
    rows = [
        ...
        _blank_row(EventKey="E5", EventType="LEAVE_HOME", EventDate="2026-05-01",
                    ChildPersonKey="C1"),
        _blank_row(EventKey="E6", EventType="DEATH", EventDate="2026-06-01",
                    SubjectPersonKey="3"),
        _blank_row(EventKey="E7", EventType="NAME_CHANGE", EventDate="2026-07-01",
                    SubjectPersonKey="1", PreviousFullName="Ava Stone",
                    NewFullName="Ava Parker", NameChangeReason="legal_name_change"),
        _blank_row(EventKey="E8", EventType="ADOPTION", EventDate="2026-08-01",
                    ChildPersonKey="C1", AdoptiveParent1PersonKey="2",
                    NewHouseholdKey="H2"),
    ]
