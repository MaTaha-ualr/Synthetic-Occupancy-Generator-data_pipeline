"""End-to-end smoke tests for all 14 canonical scenarios using shipped YAMLs.
...
def _copy_params(project_root: Path) -> None:
    ...
    dest = project_root / "phase2" / "Data" / "phase2_params"
...
def test_smoke_name_change_lifecycle(tmp_path: Path) -> None:
    result = _run_scenario(tmp_path, "name_change_lifecycle", sample_override=SAMPLE)
    assert result["validation_valid"] is True
    events = pd.read_parquet(Path(result["run_dir"]) / "truth_events.parquet")
    assert "NAME_CHANGE" in events["EventType"].str.upper().values


def test_smoke_death_survivor_persistence(tmp_path: Path) -> None:
    result = _run_scenario(tmp_path, "death_survivor_persistence", sample_override=SAMPLE)
    assert result["validation_valid"] is True
    events = pd.read_parquet(Path(result["run_dir"]) / "truth_events.parquet")
    assert "DEATH" in events["EventType"].str.upper().values


def test_smoke_adoption_blended_family(tmp_path: Path) -> None:
    ...
    assert result["validation_valid"] is True
    events = pd.read_parquet(Path(result["run_dir"]) / "truth_events.parquet")
    assert "ADOPTION" in events["EventType"].str.upper().values
