...
def test_quality_report_lifecycle_events_counted() -> None:
    people = _make_people(3)
    people["IsDeceased"] = [True, False, False]
    people["DeathDate"] = ["2026-03-01", "", ""]
    events = pd.concat(
        [
            _empty_events(),
            pd.DataFrame(
                [
                    {"EventKey": "E1", "EventType": "DEATH", "EventDate": "2026-03-01"},
                    {"EventKey": "E2", "EventType": "NAME_CHANGE", "EventDate": "2026-04-01"},
                    {"EventKey": "E3", "EventType": "ADOPTION", "EventDate": "2026-05-01"},
                ]
            ),
        ],
        ignore_index=True,
    ...
    report = compute_phase2_quality_report(
        ...
        truth_households_df=_make_households(3),
        truth_household_memberships_df=_make_memberships(3),
    ...
    event_counts = report["scenario_metrics"]["event_counts"]
    assert event_counts["deaths"] == 1
    assert event_counts["name_changes"] == 1
    assert event_counts["adoptions"] == 1
    assert report["scenario_metrics"]["lifecycle_population_counts"]["people_deceased"] == 1
    assert report["scenario_metrics"]["lifecycle_population_counts"]["people_with_death_date"] == 1
