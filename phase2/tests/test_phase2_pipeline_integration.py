...
def _write_phase2_params(project_root: Path) -> None:
    params_dir = project_root / "phase2" / "Data" / "phase2_params"
...
def test_run_scenario_pipeline_single_dataset_end_to_end(tmp_path: Path) -> None:
    ...
    assert Path(result["paths"]["master_dataset"]).exists()
    registry = _read_run_csv(result["paths"]["datasets"]["registry"])
    master = _read_run_csv(result["paths"]["master_dataset"])
    ...
    assert len(source_master) <= len(registry)
    assert not timeline_master.empty
    assert "PersonKey" in master.columns
...
def test_run_scenario_pipeline_pairwise_dataset_list_end_to_end(tmp_path: Path) -> None:
    ...
    source_master = master[master["RecordType"] == "source_snapshot"]
    assert len(source_master) <= len(registry) + len(claims)
    assert "TIMELINE" in set(master["DatasetId"].astype(str))
    master_path = result["paths"]["master_dataset"]
    assert Path(master_path).name == "masterDataset.csv"
    ...
    assert manifest["observed_outputs"]["master_dataset"] == result["paths"]["master_dataset"]
    quality = json.loads(Path(result["paths"]["quality_report"]).read_text(encoding="utf-8"))
...
def test_run_scenario_pipeline_is_reproducible_for_same_seed(tmp_path: Path) -> None:
    ...
    assert_frame_equal(
        _read_run_csv(result_a["paths"]["master_dataset"]),
        _read_run_csv(result_b["paths"]["master_dataset"]),
    )
    assert_frame_equal(
...
def test_run_scenario_pipeline_medium_scale_smoke(tmp_path: Path) -> None:
    ...
    assert result["observed_counts"]["master_source_rows"] <= 220
    assert result["observed_counts"]["master_timeline_rows"] >= 200
...
def test_run_scenario_pipeline_multi_dataset_scale_smoke(tmp_path: Path) -> None:
    ...
    assert result["observed_counts"]["master_source_rows"] <= result["observed_counts"]["entity_record_map_rows"]
    assert result["observed_counts"]["master_timeline_rows"] >= 300
