...
@pytest.mark.parametrize("scenario_id", CANONICAL_SCENARIOS)
def test_pairwise_cardinality_record_targets_do_not_erase_forced_duplicates(
    scenario_id: str,
) -> None:
    data = _load_scenario(scenario_id)
    emission = parse_emission_config(data.get("emission", {}))
    selection = parse_selection_config(data.get("selection", {}))
    if selection.sample_mode != "count":
        return

    selected_entities = int(selection.sample_value)
    if emission.crossfile_match_mode == "one_to_many" and emission.record_count_b is not None:
        assert emission.record_count_b > selected_entities
    if emission.crossfile_match_mode == "many_to_one" and emission.record_count_a is not None:
        assert emission.record_count_a > selected_entities
    if emission.crossfile_match_mode == "many_to_many":
        if emission.record_count_a is not None:
            assert emission.record_count_a > selected_entities
        if emission.record_count_b is not None:
            assert emission.record_count_b > selected_entities
