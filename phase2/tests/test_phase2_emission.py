...
def test_parse_emission_config_supports_record_count_controls() -> None:
    legacy = parse_emission_config(
        {
            "record_count_A": 12,
            "record_count_B": 17,
        }
    )
    assert legacy.record_count_a == 12
    assert legacy.record_count_b == 17
    assert legacy.datasets[0].record_count == 12
    assert legacy.datasets[1].record_count == 17

    canonical = parse_emission_config(
        {
            "crossfile_match_mode": "single_dataset",
            "datasets": [
                {
    ...
    assert canonical.datasets[0].record_count == 9


def test_parse_emission_config_rejects_invalid_record_count() -> None:
    with pytest.raises(ValueError, match="record_count_A must be an integer"):
        parse_emission_config({"record_count_A": 1.5})
    with pytest.raises(ValueError, match="record_count_B must be >= 0"):
        parse_emission_config({"record_count_B": -1})
...
def test_emit_observed_datasets_one_to_many_generates_crosswalk() -> None:
    ...
    master_dataset = emitted["master_dataset"]
    ...
    assert {"PersonKey", "DatasetId", "RecordKey"} <= set(master_dataset.columns)
    source_master = master_dataset[master_dataset["RecordType"] == "source_snapshot"]
    timeline_master = master_dataset[master_dataset["RecordType"] == "residence_timeline"]
    assert len(source_master) <= len(entity_record_map)
    assert not timeline_master.empty
    assert master_dataset.duplicated(
        subset=[column for column in master_dataset.columns if column != "RecordKey"]
    ).sum() == 0
    assert len(dataset_a) >= 3
    assert len(dataset_b) >= 3
    assert emitted["metrics"]["match_mode"] == "one_to_many"

    ordered = master_dataset.sort_values(
        by=["PersonKey", "DatasetId", "RecordKey"],
        kind="mergesort",
    ).reset_index(drop=True)
    pd.testing.assert_frame_equal(master_dataset, ordered)
...
def test_emit_observed_datasets_single_dataset_mode_omits_pairwise_crosswalk() -> None:
    ...
    source_master = master_dataset[master_dataset["RecordType"] == "source_snapshot"]
    assert set(source_master["DatasetId"].astype(str)) == {"registry"}
    assert len(source_master) <= len(entity_record_map)
    assert "TIMELINE" in set(master_dataset["DatasetId"].astype(str))
    assert master_dataset.duplicated(
        subset=[column for column in master_dataset.columns if column != "RecordKey"]
    ).sum() == 0
...
def test_emit_observed_datasets_multi_dataset_mode_omits_pairwise_crosswalk() -> None:
    ...
    source_master = emitted["master_dataset"][emitted["master_dataset"]["RecordType"] == "source_snapshot"]
    assert set(source_master["DatasetId"].astype(str)) == {"registry", "claims", "benefits"}
    assert len(source_master) <= len(emitted["entity_record_map"])
    assert "TIMELINE" in set(emitted["master_dataset"]["DatasetId"].astype(str))
    assert emitted["master_dataset"].duplicated(
        subset=[column for column in emitted["master_dataset"].columns if column != "RecordKey"]
    ).sum() == 0
