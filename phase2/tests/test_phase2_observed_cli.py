...
def test_generate_phase2_observed_cli_supports_single_dataset_mode(monkeypatch, tmp_path):
    ...
    script_path = PROJECT_ROOT / "phase2" / "scripts" / "generate_phase2_observed.py"
    ...
    master_dataset_path = run_dir / "masterDataset.csv"
    ...
    assert master_dataset_path.exists()
    ...
    assert manifest["observed_outputs"]["master_dataset"] == str(master_dataset_path)
    ...
    master_df = pd.read_csv(master_dataset_path, dtype=str).fillna("")
    ...
    source_master = master_df[master_df["RecordType"] == "source_snapshot"]
    timeline_master = master_df[master_df["RecordType"] == "residence_timeline"]
    assert len(source_master) <= len(observed_df)
    assert not timeline_master.empty
    assert "PersonKey" in master_df.columns
    assert master_df.duplicated(subset=[column for column in master_df.columns if column != "RecordKey"]).sum() == 0
...
def test_generate_phase2_observed_cli_supports_pairwise_dataset_list_mode(monkeypatch, tmp_path):
    ...
    assert len(master_df) >= len(registry_df)
    assert "TIMELINE" in set(master_df["DatasetId"].astype(str))
    assert "PersonKey" in master_df.columns
...
def test_generate_phase2_observed_cli_supports_multi_dataset_mode(monkeypatch, tmp_path):
    ...
    assert (run_dir / "masterDataset.csv").exists()
    ...
    assert manifest["observed_outputs"]["master_dataset"] == str(run_dir / "masterDataset.csv")
    assert manifest["observed_outputs"]["truth_crosswalk"] == ""
