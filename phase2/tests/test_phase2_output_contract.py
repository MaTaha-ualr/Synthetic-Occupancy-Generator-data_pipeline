...
def _write_minimal_valid_run_artifacts(runs_root: Path, run_id: str) -> dict[str, Path]:
    ...
    ).to_csv(paths["master_dataset"], index=False)
    pd.DataFrame(
...
def _write_minimal_multi_dataset_run_artifacts(runs_root: Path, run_id: str) -> Path:
    ...
    master_df = pd.read_csv(paths["master_dataset"], dtype=str)
    master_df = pd.concat(
        [
            master_df,
            pd.DataFrame(
    ...
    master_df.to_csv(paths["master_dataset"], index=False)
    ...
    manifest["observed_outputs"] = {
        ...
        "master_dataset": str(paths["master_dataset"]),
        "truth_crosswalk": "",
        "pairwise_crosswalks": [
...
def test_validate_phase2_outputs_cli_accepts_absolute_run_path(tmp_path: Path) -> None:
    runs_root = tmp_path / "custom_runs"
    run_id = build_run_id("single_movers", 20260320, "2026-03-20")
    paths = _write_minimal_valid_run_artifacts(runs_root, run_id)
    run_dir = paths["truth_people"].parent
    ...
    completed = subprocess.run(
        [
            sys.executable,
            str(PROJECT_ROOT / "phase2" / "scripts" / "validate_phase2_outputs.py"),
            "--run",
            str(run_dir),
            "--no-progress",
        ],
