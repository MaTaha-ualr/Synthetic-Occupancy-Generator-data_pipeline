...
PHASE2_STATIC_OBSERVED_OUTPUTS: dict[str, OutputSchema] = {
    ...
    "master_dataset": OutputSchema(
        filename="masterDataset.csv",
        file_format="csv",
        required_columns=(
            "PersonKey",
            "RecordKey",
...
def get_observed_output_schemas(emission_config: Any) -> dict[str, OutputSchema]:
    schemas: dict[str, OutputSchema] = {
        ...
        "master_dataset": PHASE2_STATIC_OBSERVED_OUTPUTS["master_dataset"],
    }
    datasets = list(getattr(emission_config, "datasets", ()))
...
def get_phase2_output_contract() -> dict[str, dict[str, Any]]:
    ...
    canonical_observed_outputs = {
        ...
        for logical_name, schema in PHASE2_STATIC_OBSERVED_OUTPUTS.items()
        if logical_name != "truth_crosswalk"
    }
    ...
    return {
        ...
        "observed_outputs_dynamic": {
            ...
            "master_dataset": "Always required combined observed dataset with source_snapshot and residence_timeline rows",
...
def validate_phase2_run(runs_root: Path, run_id: str) -> dict[str, Any]:
    ...
    if scenario_payload is not None and isinstance(scenario_payload, dict):
        try:
            ...
            observed_paths = {
                "entity_record_map": paths["entity_record_map"],
                "master_dataset": paths["master_dataset"],
            }
