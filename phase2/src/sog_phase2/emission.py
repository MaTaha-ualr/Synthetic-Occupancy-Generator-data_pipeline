...
MASTER_SOURCE_RECORD_TYPE = "source_snapshot"
MASTER_TIMELINE_RECORD_TYPE = "residence_timeline"
MASTER_TIMELINE_DATASET_ID = "TIMELINE"
...
def get_emission_schema() -> dict[str, Any]:
    ...
    defaults["record_count_A"] = defaults.pop("record_count_a")
    defaults["record_count_B"] = defaults.pop("record_count_b")
    defaults["noise"] = {
    ...
    return {
        ...
        "fields": {
            ...
            "record_count_A": "optional integer >= 0; exact final rows for DatasetA.csv",
            "record_count_B": "optional integer >= 0; exact final rows for DatasetB.csv",
            "datasets": [
                {
                    ...
                    "record_count": "optional integer >= 0; exact final rows for this dataset",
...
def _cohabit_middle_name(
    *,
    ...
    maiden = _text(maiden_last_name)
    current = _text(current_middle_name)
    if not maiden:
        return current
    if not current:
        return maiden
    if _hash_unit("cohabit_maiden_middle", seed, person_key, event_date) < 0.5:
        return maiden
    return current
    ...
    for idx, item in enumerate(raw):
        ...
        datasets.append(
            ObservedDatasetConfig(
                ...
                record_count=_parse_optional_record_count(
                    item.get("record_count"),
                    f"datasets[{idx}].record_count",
                ),
            )
        )
...
def _parse_optional_record_count(value: Any, field_name: str) -> int | None:
    if value is None:
        return None
    text = str(value).strip()
    if not text:
...
def parse_emission_config(raw: dict[str, Any] | None) -> EmissionConfig:
    ...
    if datasets_raw is not None:
        ...
        return EmissionConfig(
            ...
            record_count_a=datasets[0].record_count,
            record_count_b=datasets[1].record_count if len(datasets) > 1 else None,
    ...
    if parsed_mode == "single_dataset":
        ...
        dataset_a = ObservedDatasetConfig(
            ...
            record_count=_parse_optional_record_count(cfg.get("record_count_A"), "record_count_A"),
        )
        return EmissionConfig(
            ...
            record_count_a=dataset_a.record_count,
            record_count_b=None,
    ...
    parsed = EmissionConfig(
        ...
        record_count_a=_parse_optional_record_count(cfg.get("record_count_A"), "record_count_A"),
        record_count_b=_parse_optional_record_count(cfg.get("record_count_B"), "record_count_B"),
        dataset_a_noise=dataset_a_noise,
        ...
        datasets=(
            ...
            ObservedDatasetConfig(
                ...
                record_count=_parse_optional_record_count(cfg.get("record_count_B"), "record_count_B"),
            ),
        ),
...
def _apply_lifecycle_name_events_for_snapshot(
    *,
    people_df: pd.DataFrame,
    truth_events_df: pd.DataFrame | None,
    snapshot_date: date,
    seed: int,
) -> pd.DataFrame:
    if people_df.empty or truth_events_df is None or truth_events_df.empty:
        return people_df
    if not {"EventType", "EventDate"} <= set(truth_events_df.columns):
        return people_df

    people = people_df.copy()
    people["PersonKey"] = people["PersonKey"].astype(str).str.strip()
    state = {
        str(row["PersonKey"]).strip(): {
            "Gender": _text(row.get("Gender")),
            "FormalFirstName": _text(row.get("FormalFirstName")),
            "MiddleName": _text(row.get("MiddleName")),
            "LastName": _text(row.get("LastName")),
            "MaidenLastName": _text(row.get("LastName")),
        }
        for _, row in people.iterrows()
        if _text(row.get("PersonKey"))
    }

    events = truth_events_df.copy()
    events["_event_date"] = pd.to_datetime(events["EventDate"], errors="coerce")
    snap = pd.Timestamp(snapshot_date)
    event_types = events["EventType"].astype(str).str.upper()
    events = events[
        event_types.isin(["COHABIT", "NAME_CHANGE", "ADOPTION"])
        & events["_event_date"].notna()
        & (events["_event_date"] <= snap)
    ].copy()
    if events.empty:
        return people

    events = events.sort_values(by=["_event_date", "EventKey"], kind="mergesort")
    for _, event in events.iterrows():
        event_type = _text(event.get("EventType")).upper()
        if event_type in {"NAME_CHANGE", "ADOPTION"}:
            person_key = _text(event.get("SubjectPersonKey")) or _text(event.get("ChildPersonKey"))
            if person_key not in state:
                continue
            new_first = _text(event.get("NewFirstName"))
            new_middle = _text(event.get("NewMiddleName"))
            new_last = _text(event.get("NewLastName"))
            if new_first:
                state[person_key]["FormalFirstName"] = new_first
            if new_middle or "NewMiddleName" in event.index:
                state[person_key]["MiddleName"] = new_middle
            if new_last:
                state[person_key]["LastName"] = new_last
            continue

        person_a = _text(event.get("PersonKeyA"))
        person_b = _text(event.get("PersonKeyB"))
        if person_a not in state or person_b not in state:
            continue
        gender_a = _gender_bucket(state[person_a].get("Gender"))
        gender_b = _gender_bucket(state[person_b].get("Gender"))
        ...
        male_last = _text(state[male_key].get("LastName"))
        if not male_last:
            continue
        event_date = _text(event.get("EventDate"))
        state[female_key]["MiddleName"] = _cohabit_middle_name(
            person_key=female_key,
            event_date=event_date,
            seed=seed,
            current_middle_name=_text(state[female_key].get("MiddleName")),
            maiden_last_name=_text(state[female_key].get("MaidenLastName")),
        )
        state[female_key]["LastName"] = male_last

    people["FormalFirstName"] = people["PersonKey"].map(
        lambda key: _text(state.get(str(key), {}).get("FormalFirstName"))
    )
    people["MiddleName"] = people["PersonKey"].map(
        lambda key: _text(state.get(str(key), {}).get("MiddleName"))
    )
    people["LastName"] = people["PersonKey"].map(
        lambda key: _text(state.get(str(key), {}).get("LastName"))
    )
    people["FormalFullName"] = people.apply(
        lambda row: " ".join(
            part
            for part in [
                _text(row.get("FormalFirstName")),
...
def _apply_cohabit_name_changes_for_snapshot(
    *,
    people_df: pd.DataFrame,
    truth_events_df: pd.DataFrame | None,
    snapshot_date: date,
    ...
    return _apply_lifecycle_name_events_for_snapshot(
        people_df=people_df,
...
def _build_snapshot(
    *,
    ...
    truth_events_df: pd.DataFrame | None = None,
    seed: int = 0,
) -> pd.DataFrame:
    ...
    people = _apply_lifecycle_name_events_for_snapshot(
        people_df=people,
        truth_events_df=truth_events_df,
        snapshot_date=snapshot_date,
        seed=seed,
...
def _target_entity_count(
    *,
    ...
    if record_count is not None:
        return min(candidate_count, max(0, int(record_count)))
    return min(candidate_count, max(0, int(round((appearance_pct / 100.0) * candidate_count))))
...
def _choose_entity_appearance(
    *,
    ...
    n = len(base_keys)
    target_a = _target_entity_count(
        candidate_count=n,
        appearance_pct=config.appearance_a_pct,
        record_count=config.record_count_a,
    )
    target_b_base = _target_entity_count(
        candidate_count=n,
        appearance_pct=config.appearance_b_pct,
        record_count=config.record_count_b,
    )
    ...
    if config.record_count_b is not None:
        late_for_b_n = min(len(sorted_late), max(0, int(config.record_count_b) - len(b_entities)))
    else:
...
def _choose_entities_for_multiple_datasets(
    *,
    ...
    target_counts = {
        dataset.dataset_id: _target_entity_count(
            candidate_count=len(candidate_keys_by_dataset.get(dataset.dataset_id, [])),
            appearance_pct=dataset.appearance_pct,
            record_count=dataset.record_count,
        )
        for dataset in datasets
...
def _resize_record_counts_for_target(
    *,
    counts: dict[str, int],
    target: int | None,
    dataset_id: str,
    ...
    target = int(target)
    if target < 0:
        raise ValueError(f"record_count for dataset {dataset_id} must be >= 0")
    if target == 0:
        return {}

    resized = {str(key): int(value) for key, value in counts.items() if int(value) > 0}
    if not resized:
        raise ValueError(
            f"record_count for dataset {dataset_id} is {target}, but no eligible entities were selected"
        )

    current = sum(resized.values())
    while current > target:
        duplicate_keys = [key for key, value in resized.items() if value > 1]
        if duplicate_keys:
            key = sorted(duplicate_keys, key=lambda item: (-resized[item], _stable_key(item)))[0]
            resized[key] -= 1
        else:
            key = sorted(resized.keys(), key=_stable_key)[-1]
            del resized[key]
        current -= 1

    keys = sorted(resized.keys(), key=_stable_key)
    while current < target:
...
def _apply_record_count_targets(
    *,
    record_counts_by_dataset: dict[str, dict[str, int]],
    datasets: list[ObservedDatasetConfig],
    rng: np.random.Generator,
) -> dict[str, dict[str, int]]:
    adjusted: dict[str, dict[str, int]] = {}
    for dataset in datasets:
        counts = record_counts_by_dataset.get(dataset.dataset_id, {})
        adjusted[dataset.dataset_id] = _resize_record_counts_for_target(
            counts=counts,
            target=dataset.record_count,
            dataset_id=dataset.dataset_id,
            rng=rng,
        )
    return adjusted
...
def _refresh_coverage_for_selected_sets(
    *,
    ...
    late_set = set(late_only_keys)

    refreshed["dataset_entities"] = {
        dataset_id: int(len(selected_sets.get(dataset_id, set())))
        for dataset_id in dataset_ids
    }
    refreshed["candidate_entities"] = {
        dataset.dataset_id: int(len(base_set | late_set))
        if dataset.snapshot == "simulation_end"
        else int(len(base_set))
        for dataset in datasets
    }

    selected_union = set().union(*(selected_sets.get(dataset_id, set()) for dataset_id in dataset_ids)) if dataset_ids else set()
    refreshed["union_entities"] = int(len(selected_union))

    if len(dataset_ids) == 1:
        return refreshed

    selected_all_overlap = set.intersection(
        *(selected_sets.get(dataset_id, set()) for dataset_id in dataset_ids)
    ) if dataset_ids else set()
    refreshed["all_dataset_overlap_entities"] = int(len(selected_all_overlap))

    pairwise_overlap: dict[str, Any] = {}
    for idx, first_id in enumerate(dataset_ids):
        for second_id in dataset_ids[idx + 1:]:
            pair_overlap = selected_sets.get(first_id, set()) & selected_sets.get(second_id, set())
            pair_union = selected_sets.get(first_id, set()) | selected_sets.get(second_id, set())
            pairwise_overlap[f"{first_id}__{second_id}"] = {
                "dataset_ids": [first_id, second_id],
                "overlap_entities": int(len(pair_overlap)),
                "union_entities": int(len(pair_union)),
    ...
    refreshed["pairwise_overlap"] = pairwise_overlap

    if len(dataset_ids) == 2:
        first_id, second_id = dataset_ids
        first_set = selected_sets.get(first_id, set())
        second_set = selected_sets.get(second_id, set())
        overlap = first_set & second_set
        refreshed.update(
            {
                "a_entities_base": int(len(first_set & base_set)),
                "b_entities_base": int(len(second_set & base_set)),
                "overlap_entities": int(len(overlap)),
                "a_only_entities": int(len(first_set - overlap)),
                "b_only_entities": int(len(second_set - overlap)),
                "late_only_in_b": int(len(second_set & late_set)),
            }
        )
...
def _build_residence_timeline_master_rows(
    *,
    truth_people_df: pd.DataFrame,
    truth_residence_history_df: pd.DataFrame,
    address_book: dict[str, dict[str, str]],
    ...
    base_columns = [
        ...
        "Phone",
        "AddressKey",
        *ADDRESS_DETAIL_COLUMNS,
        "SourceSnapshotDate",
        "ResidenceStartDate",
        "ResidenceEndDate",
        "SourceSystem",
    ]
    if truth_people_df.empty or truth_residence_history_df.empty:
        return pd.DataFrame(columns=base_columns)

    residence = truth_residence_history_df.copy()
    residence["PersonKey"] = residence["PersonKey"].astype(str).str.strip()
    residence["AddressKey"] = residence["AddressKey"].astype(str).str.strip()
    residence["ResidenceStartDate"] = residence["ResidenceStartDate"].astype(str)
    residence["ResidenceEndDate"] = residence["ResidenceEndDate"].astype(str)
    residence["_start"] = pd.to_datetime(residence["ResidenceStartDate"], errors="coerce")
    residence = residence.sort_values(
        by=["PersonKey", "_start", "AddressKey"],
        key=lambda s: s.map(_stable_key) if s.name == "PersonKey" else s,
        kind="mergesort",
    )
    ...
    def people_by_date(snapshot_date: date) -> dict[str, pd.Series]:
        ...
        if cache_key not in people_cache:
            snapshot_people = _apply_lifecycle_name_events_for_snapshot(
                people_df=truth_people_df.copy(),
                truth_events_df=truth_events_df,
                snapshot_date=snapshot_date,
                seed=seed,
            )
            snapshot_people["PersonKey"] = snapshot_people["PersonKey"].astype(str).str.strip()
            people_cache[cache_key] = {
                _text(row.get("PersonKey")): row
                for _, row in snapshot_people.iterrows()
                if _text(row.get("PersonKey"))
            }
        return people_cache[cache_key]

    rows: list[dict[str, Any]] = []
    record_counter = 0
    for _, residence_row in residence.iterrows():
        person_key = _text(residence_row.get("PersonKey"))
        if not person_key:
            continue
        start_text = _text(residence_row.get("ResidenceStartDate"))
        parsed_start = pd.to_datetime(start_text, errors="coerce")
        start_date = simulation_start_date if pd.isna(parsed_start) else parsed_start.date()
        person_row = people_by_date(start_date).get(person_key)
        if person_row is None:
            continue

        address_key = _text(residence_row.get("AddressKey"))
        address_details = address_book.get(address_key, _default_address_details())
        house_number = _text(address_details.get("HouseNumber"))
        street_name = _text(address_details.get("StreetName"))
        unit_type = _text(address_details.get("UnitType"))
        unit_number = _text(address_details.get("UnitNumber"))
        street_address = _text(address_details.get("StreetAddress")) or _compose_street_address(
            house_number,
            street_name,
            unit_type,
            unit_number,
        )
        first_name = _text(person_row.get("FormalFirstName"))
        middle_name = _text(person_row.get("MiddleName"))
        last_name = _text(person_row.get("LastName"))
        full_name = " ".join(part for part in [first_name, middle_name, last_name] if part).strip()
        record_counter += 1
        rows.append(
            {
                "PersonKey": person_key,
                "RecordKey": f"{MASTER_TIMELINE_DATASET_ID}-{record_counter:09d}",
                "DatasetId": MASTER_TIMELINE_DATASET_ID,
                "RecordType": MASTER_TIMELINE_RECORD_TYPE,
                "FirstName": first_name,
                "MiddleName": middle_name,
                "LastName": last_name,
                "Suffix": _text(person_row.get("Suffix")),
                "FullName": full_name,
                "Gender": _text(person_row.get("Gender")),
                "Ethnicity": _text(person_row.get("Ethnicity")),
                "DOB": _text(person_row.get("DOB")),
                "Age": _age_on_snapshot(
                    _text(person_row.get("DOB")),
                    start_date,
                    int(pd.to_numeric(pd.Series([person_row.get("Age")]), errors="coerce").fillna(0).iloc[0]),
                ),
                "SSN": _text(person_row.get("SSN")),
                "Phone": _text(person_row.get("Phone")),
                "AddressKey": address_key,
                "HouseNumber": house_number,
                "StreetName": street_name,
                "UnitType": unit_type,
                "UnitNumber": unit_number,
                "StreetAddress": street_address,
                "City": _text(address_details.get("City")),
                "State": _text(address_details.get("State")),
                "ZipCode": _text(address_details.get("ZipCode")),
                "SourceSnapshotDate": start_date.isoformat(),
                "ResidenceStartDate": start_date.isoformat(),
                "ResidenceEndDate": _text(residence_row.get("ResidenceEndDate")),
                "SourceSystem": "truth_residence_history",
            }
        )

    timeline = pd.DataFrame(rows, columns=base_columns)
    if not timeline.empty:
        timeline = timeline.drop_duplicates(
            subset=[column for column in timeline.columns if column != "RecordKey"],
            keep="first",
        )
        timeline = timeline.sort_values(
            by=["PersonKey", "ResidenceStartDate", "RecordKey"],
            key=lambda s: s.map(_stable_key) if s.name == "PersonKey" else s,
            kind="mergesort",
        ).reset_index(drop=True)
    return timeline
...
def build_master_dataset(
    ...
    for dataset_id in sorted(dataset_frames.keys(), key=_stable_key):
        frame = dataset_frames[dataset_id]
        if frame is None or frame.empty:
            continue
        working = frame.copy()
        if "PersonKey" in working.columns:
            working = working.drop(columns=["PersonKey"])
        if "DatasetId" not in working.columns:
            working["DatasetId"] = dataset_id
        working["RecordType"] = MASTER_SOURCE_RECORD_TYPE
        if "ResidenceStartDate" not in working.columns:
            working["ResidenceStartDate"] = ""
        if "ResidenceEndDate" not in working.columns:
            working["ResidenceEndDate"] = ""
        frames.append(working)

    base_columns = ["PersonKey", "RecordKey", "DatasetId", "RecordType"]
    if frames:
        source_stack = pd.concat(frames, ignore_index=True, sort=False)
        map_df = entity_record_map.copy()
        if not map_df.empty:
            map_df = map_df[["PersonKey", "DatasetId", "RecordKey"]].drop_duplicates(
                subset=["DatasetId", "RecordKey"],
                keep="first",
            )
            source_stack = source_stack.merge(
                map_df,
                on=["DatasetId", "RecordKey"],
                how="left",
                validate="one_to_one",
            )
        else:
            source_stack["PersonKey"] = ""

        source_stack["PersonKey"] = source_stack["PersonKey"].map(_text)
        dedupe_columns = [column for column in source_stack.columns if column != "RecordKey"]
        if dedupe_columns:
            source_stack = source_stack.drop_duplicates(subset=dedupe_columns, keep="first")
    else:
        source_stack = pd.DataFrame(columns=base_columns)

    timeline = (
        timeline_frame.copy()
        if timeline_frame is not None and not timeline_frame.empty
    ...
    if not timeline.empty:
        for column in base_columns:
            if column not in timeline.columns:
                timeline[column] = ""
        timeline["DatasetId"] = timeline["DatasetId"].map(_text).replace("", MASTER_TIMELINE_DATASET_ID)
        timeline["RecordType"] = timeline["RecordType"].map(_text).replace("", MASTER_TIMELINE_RECORD_TYPE)
        timeline["PersonKey"] = timeline["PersonKey"].map(_text)

    if source_stack.empty and timeline.empty:
        return pd.DataFrame(columns=base_columns)

    combined = pd.concat([source_stack, timeline], ignore_index=True, sort=False)
    combined["PersonKey"] = combined["PersonKey"].map(_text)
    combined["DatasetId"] = combined["DatasetId"].map(_text)
    combined["RecordType"] = combined["RecordType"].map(_text)
    dedupe_columns = [column for column in combined.columns if column != "RecordKey"]
    if dedupe_columns:
    ...
    master = combined[ordered_columns]
...
def emit_observed_datasets(
    *,
    ...
    record_counts_by_dataset = _apply_record_count_targets(
        record_counts_by_dataset=record_counts_by_dataset,
        datasets=datasets,
        rng=rng,
    )
    selected_sets = {
        dataset.dataset_id: set(record_counts_by_dataset.get(dataset.dataset_id, {}).keys())
        for dataset in datasets
    }
    if len(datasets) >= 2:
        first_dataset, second_dataset = datasets[0], datasets[1]
        ...
        first_only_keys = selected_sets[first_dataset.dataset_id] - overlap_keys
        second_only_keys = selected_sets[second_dataset.dataset_id] - overlap_keys
    coverage_counts = _refresh_coverage_for_selected_sets(
    ...
    timeline_frame = _build_residence_timeline_master_rows(
        truth_people_df=truth_people_df,
        truth_residence_history_df=truth_residence_history_df,
        address_book=address_book,
    ...
    master_dataset = build_master_dataset(dataset_frames, entity_record_map, timeline_frame=timeline_frame)
    source_dataset_rows = int(sum(len(frame) for frame in dataset_frames.values()))
    master_source_rows = int(
        (master_dataset.get("RecordType", pd.Series(dtype=str)).astype(str) == MASTER_SOURCE_RECORD_TYPE).sum()
    )
    master_timeline_rows = int(
        (master_dataset.get("RecordType", pd.Series(dtype=str)).astype(str) == MASTER_TIMELINE_RECORD_TYPE).sum()
    )
    ...
    metrics = {
        ...
        "master_dataset_rows": int(len(master_dataset)),
        "source_dataset_rows": source_dataset_rows,
        "master_source_rows": master_source_rows,
        "master_timeline_rows": master_timeline_rows,
        "master_dataset_deduped_rows": int(max(0, source_dataset_rows - master_source_rows)),
