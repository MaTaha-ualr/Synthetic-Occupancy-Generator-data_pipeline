    ...
    *,
    ...
    scenario_metrics = {
        "event_counts": {
            ...
            "leave_home": int(event_counts.get("LEAVE_HOME", 0)),
            "deaths": int(event_counts.get("DEATH", 0)),
            "name_changes": int(event_counts.get("NAME_CHANGE", 0)),
            "adoptions": int(event_counts.get("ADOPTION", 0)),
        },
        "lifecycle_population_counts": {
            "people_deceased": int(
                truth_people_df.get("IsDeceased", pd.Series(dtype=object))
                .astype(str)
                .str.lower()
                .isin(["true", "1", "yes"])
                .sum()
            ),
            "people_with_death_date": int(
                truth_people_df.get("DeathDate", pd.Series(dtype=object)).astype(str).str.strip().ne("").sum()
            ),
        },
