...
def _mobility_agebin_base_map(mobility_params_df: pd.DataFrame) -> dict[str, float]:
    ...
    has_population = "population" in mobility_params_df.columns
    cohort_pop = (
        {
            str(row["age_cohort_id"]): float(row.get("population", 0.0) or 0.0)
            for _, row in mobility_params_df.iterrows()
        }
        if has_population
        else {}
    ...
    pop_18_24 = cohort_pop.get("age_18_24", 0.0)
    pop_25_34 = cohort_pop.get("age_25_34", 0.0)
    total_pop = pop_18_24 + pop_25_34
    if total_pop > 0:
        weighted_18_34 = ((age_18_24 * pop_18_24) + (age_25_34 * pop_25_34)) / total_pop
    else:
        # Population weights unavailable (column missing or all zero) — fall back to an
        # unweighted mean so the 18–34 mobility prior never silently collapses to zero.
        weighted_18_34 = (age_18_24 + age_25_34) / 2.0
