...
def run_scenario_pipeline(
    *,
    ...
    # ------------------------------------------------------------------
    with progress.step("Load scenario and configure run"):
    ...
    # Stage 2: Selection
    # ------------------------------------------------------------------
    with progress.step("Select scenario population"):
    ...
    # ------------------------------------------------------------------
    with progress.step("Simulate truth layer"):
        truth_outputs = {
    ...
    # ------------------------------------------------------------------
    with progress.step("Compute quality and write metadata"):
        generated_at_utc = (
    ...
    # ------------------------------------------------------------------
    with progress.step("Validate contract and write README"):
        validation = validate_phase2_run(runs_root.resolve(), run_id)

        # Write a human-readable per-run README summarizing this run's artifacts.
        # Additive only: it reads the manifest/quality/scenario files just written.
