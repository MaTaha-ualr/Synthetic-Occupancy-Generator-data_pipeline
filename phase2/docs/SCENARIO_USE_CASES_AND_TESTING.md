...
| Scenario | Best For | Truth Event Surface | Observed Match Mode | Main ER Stress |
|---|---|---|---|---|
...
| `couple_merge` | Household formation and co-residence | `COHABIT` plus `MOVE` | `one_to_many` | one person linked to multiple B records |
| `family_birth` | Household growth and new child entities | `BIRTH` plus `COHABIT` | `many_to_one` | multiple A records converging to one B record |
...
| `roommates_split` | Young-adult churn and noisy splits | `LEAVE_HOME`, `MOVE`, and supporting `COHABIT` | `one_to_many` | high duplication and unstable household context |
...
| `name_change_lifecycle` | Legal/marriage/divorce name-change continuity | `NAME_CHANGE` | `one_to_one` | true identity persists while observed names change |
| `death_survivor_persistence` | Death lifecycle and stale record persistence | `DEATH` | `one_to_one` | deceased truth state with downstream observed records |
| `adoption_blended_family` | Adoption and blended-family transitions | `ADOPTION` | `one_to_one` | child household, parent, address, and surname changes |
...
- Need legal name continuity: run `name_change_lifecycle`.
- Need death lifecycle or stale survivor-record behavior: run `death_survivor_persistence`.
- Need adoption or blended-family relationship changes: run `adoption_blended_family`.
...
- `Parent2PersonKey` populated for partnered births after cohabitation
...
|---|---|---|---|
| `single_movers` | `phase2/scenarios/single_movers.yaml` | `phase2/tests/test_phase2_scenario_regression.py` | `MOVE` events and overlap math |
| `clean_baseline_linkage` | `phase2/scenarios/clean_baseline_linkage.yaml` | `phase2/tests/test_phase2_scenario_regression.py` | low-noise one-to-one baseline behavior |
| `couple_merge` | `phase2/scenarios/couple_merge.yaml` | `phase2/tests/test_phase2_scenario_regression.py` | `COHABIT`, shared residence, one-to-many behavior |
| `family_birth` | `phase2/scenarios/family_birth.yaml` | `phase2/tests/test_phase2_scenario_regression.py` | `BIRTH`, child creation, many-to-one behavior |
| `divorce_custody` | `phase2/scenarios/divorce_custody.yaml` | `phase2/tests/test_phase2_scenario_regression.py` | `DIVORCE`, post-divorce households, many-to-many behavior |
| `roommates_split` | `phase2/scenarios/roommates_split.yaml` | `phase2/tests/test_phase2_scenario_regression.py` | roommate grouping, `LEAVE_HOME`, one-to-many behavior |
| `high_noise_identity_drift` | `phase2/scenarios/high_noise_identity_drift.yaml` | `phase2/tests/test_phase2_scenario_regression.py` | heavy B-side identity drift with low structural ambiguity |
| `low_overlap_sparse_coverage` | `phase2/scenarios/low_overlap_sparse_coverage.yaml` | `phase2/tests/test_phase2_scenario_regression.py` | low overlap and large unmatched populations |
| `asymmetric_source_coverage` | `phase2/scenarios/asymmetric_source_coverage.yaml` | `phase2/tests/test_phase2_scenario_regression.py` | strong A-side coverage imbalance |
| `high_duplication_dedup` | `phase2/scenarios/high_duplication_dedup.yaml` | `phase2/tests/test_phase2_scenario_regression.py` | single-dataset topology and high duplicate pressure |
| `three_source_partial_overlap` | `phase2/scenarios/three_source_partial_overlap.yaml` | `phase2/tests/test_phase2_scenario_regression.py` | N-way topology, pairwise crosswalk inventory, and partial overlap |

Quality-report completeness is covered by `phase2/tests/test_phase2_quality.py`.

Single-dataset emission behavior is covered separately by:

- `phase2/tests/test_phase2_emission.py`
- `phase2/tests/test_phase2_output_contract.py`
...
- `phase2/docs/USE_CASE_GUIDE.md`: one worked DataLink example
