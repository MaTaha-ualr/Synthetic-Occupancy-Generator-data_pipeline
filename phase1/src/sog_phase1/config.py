...
def validate_phase1_core(phase1: dict[str, Any]) -> None:
    ...
    if not redundancy_enabled and n_records != n_people:
        raise ValueError("When redundancy.enabled=false, phase1.n_records must equal phase1.n_people")
    if redundancy_enabled and n_records <= n_people:
        raise ValueError("When redundancy.enabled=true, phase1.n_records must be greater than phase1.n_people")
    min_total = n_people * min_records_per_entity
    max_total = n_people * max_records_per_entity
    if n_records < min_total or n_records > max_total:
        raise ValueError(
            "phase1.n_records must satisfy "
            f"n_people*min_records_per_entity <= n_records <= n_people*max_records_per_entity "
    ...
    for key in ("middle_name", "suffix", "phone"):
        ...
        if val < 0 or val > 100:
            raise ValueError(f"fill_rates.{key} must be between 0 and 100")
