...
def _uppercase_text_columns(df: pd.DataFrame) -> pd.DataFrame:
    text_columns = df.select_dtypes(include=["object", "string"]).columns
    for column in text_columns:
        df[column] = df[column].map(_uppercase_text_value)
    return df


def _uppercase_mapping_keys(values: dict[str, Any]) -> dict[str, Any]:
    return {str(key).upper(): value for key, value in values.items()}


def _uppercase_age_bin_metadata(age_bins: list[dict[str, Any]]) -> list[dict[str, Any]]:
    normalized_bins: list[dict[str, Any]] = []
    for item in age_bins:
        normalized = dict(item)
        for field in ("id", "label"):
            if field in normalized:
                normalized[field] = _uppercase_text_value(normalized[field])
        normalized_bins.append(normalized)
...
def generate_phase1_dataset(
    *,
    ...
    middle_fill_rate = float(fill_rates.get("middle_name", 0.0)) / 100.0
    suffix_fill_rate = float(fill_rates.get("suffix", 0.0)) / 100.0
    phone_fill_rate = float(fill_rates.get("phone", 100.0)) / 100.0
    ...
    entity_person_keys = np.arange(1, n_people + 1, dtype=np.int64)
    entity_genders = _draw_exact_categories(gender_keys, gender_probs, n_people, rng)
    entity_ethnicities = _draw_exact_categories(ethnicity_keys, ethnicity_probs, n_people, rng)
    entity_age_bins = _draw_exact_categories(age_keys, age_probs, n_people, rng)

    formal_first_names = np.empty(n_people, dtype=object)
    ...
    entity_last_names = np.empty(n_people, dtype=object)
    ...
    middle_fill_mask = rng.random(n_people) < middle_fill_rate
    entity_middle_names = np.array([""] * n_people, dtype=object)
    sampled_middle_names = rng.choice(middle_name_pool.names, size=n_people, p=middle_name_pool.probs)
    entity_middle_names[middle_fill_mask] = sampled_middle_names[middle_fill_mask]

    suffix_fill_mask = rng.random(n_people) < suffix_fill_rate
    entity_suffixes = np.array([""] * n_people, dtype=object)
    ...
    entity_dobs = np.empty(n_people, dtype=object)
    entity_ages = np.empty(n_people, dtype=np.int64)
    ...
    entity_phone = np.array([""] * n_people, dtype=object)
    phone_fill_mask = rng.random(n_people) < phone_fill_rate
    ...
    if nickname_enabled and nickname_mode == "per_person":
        ...
        entity_use_mask = rng.random(n_people) < nickname_usage_rate
        for i in range(n_people):
    ...
    normalized_gender_raw_pct = _uppercase_mapping_keys(gender_norm.raw_percentages)
    normalized_gender_prob = _uppercase_mapping_keys(gender_norm.probabilities)
    normalized_gender_pct = _uppercase_mapping_keys(gender_norm.normalized_percentages)
    normalized_ethnicity_raw_pct = _uppercase_mapping_keys(ethnicity_norm.raw_percentages)
    normalized_ethnicity_prob = _uppercase_mapping_keys(ethnicity_norm.probabilities)
    normalized_ethnicity_pct = _uppercase_mapping_keys(ethnicity_norm.normalized_percentages)
    normalized_age_raw_pct = _uppercase_mapping_keys(age_norm.raw_percentages)
    ...
    manifest = {
        ...
        "normalization": {
            "gender": {
                "raw_percentages": normalized_gender_raw_pct,
                "raw_sum": gender_norm.raw_sum,
                "normalized": normalized_gender_prob,
                "normalized_percentages": normalized_gender_pct,
            },
            "ethnicity": {
                "raw_percentages": normalized_ethnicity_raw_pct,
                "raw_sum": ethnicity_norm.raw_sum,
                "normalized": normalized_ethnicity_prob,
                "normalized_percentages": normalized_ethnicity_pct,
            },
            "age_bins": {
                "raw_percentages": normalized_age_raw_pct,
                "raw_sum": age_norm.raw_sum,
                "normalized": normalized_age_prob,
                "normalized_percentages": normalized_age_pct,
                "selected_bins": normalized_selected_age_bins,
    ...
    quality_report = {
        ...
        "expected_distributions_pct": {
            ...
            "ethnicity": normalized_ethnicity_pct,
            "age_bins": normalized_age_pct,
        },
