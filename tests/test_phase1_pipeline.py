...
def test_phase1_legacy_n_people_compatibility(tmp_path: Path) -> None:
    cfg = _base_config()
    phase1 = cfg["phase1"]

    phase1.pop("n_entities", None)
    phase1.pop("n_records", None)
    phase1["n_people"] = 5000
    phase1["seed"] = 20260305

    phase1["redundancy"]["enabled"] = False
    phase1["redundancy"]["min_records_per_entity"] = 1
    phase1["redundancy"]["max_records_per_entity"] = 1

    phase1["nicknames"]["enabled"] = False
    phase1["name_duplication"]["exact_full_name_people_pct"] = 20.0

    result, df = _run_generation(cfg, tmp_path, "legacy_people_addresses.csv")

    assert len(df) == 5000
    assert int(result["n_entities"]) == 5000
    assert int(result["n_records"]) == 5000
    assert int(result["n_people"]) == 5000
    ...
    assert (df["FormalFirstName"] == df["FirstName"]).all()
    assert (df["FormalFullName"] == df["FullName"]).all()
    assert set(df["FirstNameType"].fillna("").unique().tolist()) == {"FORMAL"}
...
def test_phase1_redundancy_with_per_record_nicknames(tmp_path: Path) -> None:
    ...
    assert len(df) == 4500
    assert int(result["n_entities"]) == 3000
